...
class SSP
{
	...
	static function containsSpecialCharacters($str) {
	...
	static function filter($request, $columns, &$bindings)
	{
		...
		if (isset($request['search']) && $request['search']['value'] != '') {
			...
			if(!self::containsSpecialCharacters($str)){
				$str = '%' . $str . '%';
			}
	...
	static function sql_connect($sql_details)
	{
		try {
			$db = @new PDO(
				"mysql:host={$sql_details['Database_Server']};dbname={$sql_details['Database_Name']}",
				$sql_details['User_Id'],
	...
	static function actual_query($bindings, $sql = null)
	{
		if (is_array($bindings)) {
			$ien = count($bindings);
			for ($i = 0; $i < $ien; $i++) {
